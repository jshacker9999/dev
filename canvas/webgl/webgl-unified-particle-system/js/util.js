...
window.drawReadyModel = function (m) {
  ...
  gl.drawElements(
    m.gltf.mode,
    m.gltf.indices.length,
    m.gltf.indicesComponentType,
    0
  );
};
...
window.getScissorForLight = (function () {
  ...
  return function (view, proj, l) {
    ...
    if (maxpt.x < -1 || 1 < minpt.x || maxpt.y < -1 || 1 < minpt.y) {
      return null;
    }
...
window.abortIfFramebufferIncomplete = function (fbo) {
  ...
  if (fbstatus !== gl.FRAMEBUFFER_COMPLETE) {
    abort(
      "framebuffer incomplete: " + WebGLDebugUtils.glEnumToString(fbstatus)
    );
  }
};
